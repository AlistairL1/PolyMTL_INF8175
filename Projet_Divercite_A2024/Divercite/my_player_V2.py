...
class MyPlayer(PlayerDivercite):
    ...
    def compute_action(self, current_state: GameState, remaining_time: int = 1e9, **kwargs) -> Action:
        ...
        if current_state.get_step() >= 32:
            print("Exhaustive search triggered")
            best_value, best_action = self.exhaustive_search(current_state)
            return best_action
        elif current_state.get_step() <= 1:
            possible_actions = current_state.get_possible_light_actions()
            return random.choice(list(possible_actions))
        elif current_state.get_step() > 1 and current_state.get_step() <= 22:
            best_value, best_action = self.maxValue(current_state, float('-inf'), float('inf'), depth=2)
            return best_action
        else:
